...
def read_parameter_file(dataver):
    ...
    with StringIO(fzip.read(filename_zip).decode('utf16')) as fp:
        ...
        for line in fp.readlines():
            ...
            if tmp1[0] == '1':  #记录的开始行
                ki +=1
                if ki==1:   #补一个13行。然后打印一下 PAR_offset
                    PAR_offset['13']=[ offset , 1 ]  #记录各组记录，应在整条记录的位置
                    offset += 1  #总偏移
            ...
            if ki==0:  #文件头,记录的注释头
                if tmp1[0] in PAR_offset: #文件头中不应该有重复
                    raise(Exception('ERROR, duplicate "%s" in PAR_offset' % (tmp1[0]) ))
                else:
                    PAR_offset[ tmp1[0] ]=[ offset , len(tmp2) ]  #记录各组记录，应在整条记录的位置
                    offset += len(tmp2)  #总偏移
            #记录完整的一个参数的记录
            if tmp1[0] not in one_par: #有的行号，有多条记录, 比如 '6|Raw Value, Corresponding text'
                one_par[ tmp1[0] ]=[]
                for jj in tmp2:
...
if __name__=='__main__':
    if(len(sys.argv)<2):
        ...
        exit()
    try:
        opts, args = getopt.gnu_getopt(sys.argv[1:],'hvp:f:l',['help','ver=','csv=','paramlist','param='])
